...
func _run() -> void:
	...
	_check(registered_context.get("bench", []).size() == 7, "managed context should contain seven bench players")
	...
	_check(String(league.team_contexts["kocaelispor"].get("bench", [])[0].get("display_name", "")) != "", "play action should keep bench context")
	_check(result_label.text.contains("Yönetilen maç:"), "match center should show managed match statistics")
	_check(result_label.text.contains("Oyuncu değişikliği:"), "match center should format substitution events distinctly")
